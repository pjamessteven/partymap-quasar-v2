...
import { Location } from 'src/types/autogen_types';
const { t } = i18n.global;
...
export default {
  ...
  getGeneralLocationString: (location: Location): string => {
    if (location?.locality) {
      if (location.locality.region?.long_name) {
        if (location.locality.country.short_name === 'US') {
          return (
            location.locality.long_name +
            ', ' +
            location.locality.region.long_name
          );
        } else {
          return (
            location.locality.long_name +
            ', ' +
            location.locality.country.long_name
          );
        }
      } else {
        return location?.locality?.long_name;
      }
    } else {
